...
static FILE* trp_debug = nullptr;
static trampoline_type** trp_table = nullptr;
static size_t* trp_size = nullptr;
static struct sigaction const* trp_old = nullptr;
static _Thread_local _Atomic(void const*) trp_data = (void*)nullptr;
...
[[maybe_unused]]
static void sighandler_report(int signo, siginfo_t *si, void* c) {
...
void trp_start(void) {
  ...
  sigaltstack(0, &prev);
  size_t const trampoline_stksz = 10*SIGSTKSZ;
  stack_t alt = {
    .ss_sp = prev.ss_sp ? nullptr : calloc(trampoline_stksz, 1),
    .ss_size = trampoline_stksz,
  };
  if (!prev.ss_sp) {
    sigaltstack(&alt, 0);
  }
...
int main(int argc,char ** argv)
{
  ...
  unsigned long cntrl = (argc > 1 ? strtoul(argv[1], nullptr, 0) : 0);
